...
FastMCP CLI supports passing command line arguments to servers using the `--server-arg` option, enabling configuration like file paths, database connections, and debug flags:

```bash
fastmcp run server.py --server-arg="--config" --server-arg="config.json"
```

This works with all CLI commands:

```bash
# Run with arguments
fastmcp run server.py --server-arg="--database-path" --server-arg="/tmp/db.sqlite"

# Development with arguments  
fastmcp dev server.py --server-arg="--debug" --server-arg="--config" --server-arg="dev.json"

# Install with arguments for proper server discovery
fastmcp install server.py --server-arg="--config" --server-arg="prod.json"
```

To accept arguments in your server, use standard Python argument parsing at the module level:
...
parser.add_argument("--database-path", help="Database file path")
parser.add_argument("--debug", action="store_true", help="Enable debug mode")

args = parser.parse_args()

# Configure server based on arguments
mcp = FastMCP(f"MyServer{' (Debug)' if args.debug else ''}")
...
The CLI temporarily modifies `sys.argv` before importing your server module, allowing argument parsing to work exactly like running `python server.py --args` directly.
